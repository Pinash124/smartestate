...
export default function ListingsPage() {
  ...
  const [loading, setLoading] = useState(true)// ADD — sort + boost
...
const handleBoost = async (listingId: string) => {
  try {
    setBoostLoading(listingId)

    await apiRequest(`/listingBoosts/${listingId}`, {
      method: 'POST',
      auth: true,
    })

    alert('Boost thành công! -10 điểm')
  } catch {
    alert('Không đủ điểm hoặc lỗi boost')
  } finally {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          <p className="text-sm text-gray-500">{/* ADD — sort dropdown */}
<select
  value={sort}
  onChange={(e) => setSort(e.target.value as any)}
  className="ml-4 border border-gray-200 rounded-xl px-3 py-2 text-sm"
>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative h-52 overflow-hidden bg-gray-100">
                    ...
                    {/*  ADD BOOST BADGE HERE */}
                    {listing.isBoosted && (
                      <span className="absolute bottom-3 left-3 bg-amber-500 text-white text-xs px-2 py-1 rounded">
                        BOOST
                      </span>
                    )}
                  ...
                  <div className="p-4 sm:p-5">
                    ...
                    <p className="text-lg font-extrabold text-amber-600 mb-3">
                      ...
                      <button
                        ...
                        disabled={boostLoading === listing.id}
                        className="mt-2 w-full bg-amber-500 hover:bg-amber-600 text-white text-xs py-2 rounded-lg"
                      >
                        {boostLoading === listing.id ? 'Đang boost...' : ' Boost (-10 điểm)'}
                      </button>
                    </p>
